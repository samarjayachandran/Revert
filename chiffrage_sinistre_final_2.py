...
def render_manage_companies():
    ...
    # ---- Sauvegarde et rechargement de l'annuaire ----
    st.markdown("##### 💾 Sauvegarde et rechargement de l'annuaire")
    st.caption(
        "Sur Streamlit Cloud, les modifications sont réinitialisées à chaque "
        "redéploiement. Téléchargez le fichier après vos modifications pour le "
        "conserver, puis rechargez-le à la prochaine session."
    )

    # Bouton de téléchargement du fichier actuel
    file_path = Path(__file__).parent / COMPANIES_FILE
    if file_path.exists():
        with open(file_path, "rb") as f:
            excel_bytes = f.read()
        st.download_button(
            "📥 Télécharger l'annuaire actuel (.xlsx)",
            data=excel_bytes,
            file_name=COMPANIES_FILE,
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="dl_companies",
        )

    # Chargement d'un fichier de remplacement
    uploaded_companies = st.file_uploader(
        "Charger une version sauvegardée de l'annuaire (.xlsx)",
        type=["xlsx"],
        key="upload_companies",
    )
    if uploaded_companies is not None:
        try:
            incoming = pd.read_excel(uploaded_companies)
            incoming.columns = [str(c).strip() for c in incoming.columns]
            for col in incoming.columns:
                incoming[col] = incoming[col].fillna("")
            st.success(
                f"Fichier chargé : **{len(incoming)}** entreprise(s), "
                f"**{len(incoming.columns)}** colonnes."
            )
            st.dataframe(incoming.head(5), use_container_width=True, hide_index=True)
            if st.button(
                "✅ Remplacer l'annuaire par ce fichier",
                key="confirm_upload_companies",
                type="primary",
            ):
                _save_companies(incoming)
                st.success("✅ Annuaire remplacé. Rechargement en cours…")
                st.rerun()
        except Exception as e:
            st.error(f"Erreur lors du chargement : {e}")
